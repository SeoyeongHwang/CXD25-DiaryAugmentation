...
extract_template = PromptTemplate(
    ...
    template=(
        ...
        주어진 글을 당신의 일기라고 생각하고 읽고, 드러난 사건이나 일어난 것 중에서 좀 더 긍정적이거나 감사하게 바라볼 수 있는 부분을 찾으세요.
        {value}라는 가치에 해당하는 내용이 부족한 경우 {life_orientation} 관점을 바탕으로 생각하세요.
...
judge_template = PromptTemplate(
    ...
    template=(
        ...
        사용자가 거부감을 느낄 수 있으니, 실제로 하루에 일어난 일에 비해 너무 과장된 해석이지는 않은지 주의하여 살펴보세요.

        평가할 포인트:
        {point_json}

        이 긍정적/감사한 점이 '{life_orientation}' 관점과 관련이 있는지 판단하고 그 이유를 설명하세요.
